    ...
    [NetworkHelper postWithAPI:orders_list parameter:@{@"p":@(pageIndex),@"type":@(type)} successBlock:^(id response) {
     ...
     DLog(@"%@",response)
            ...
            for (NSDictionary *dic in arr) {
                XBBObject *order = [[XBBObject alloc] init];
...
- (void)segment
{
    ...
    selectTimeSegment = [[UISegmentedControl alloc] initWithItems:@[@"即刻单",@"预约单"]];
...
- (void)addTableViews
{
    ...
    [self.tableView setContentInset:UIEdgeInsetsMake(-40, 0, -40, 0)];
    self.tableView.dataSource = self;
    self.tableView.delegate = self;
    [self.view addSubview:self.tableView];
    nodataView = [[XBBNotDataView alloc] initWithFrame:self.tableView.bounds withImage:[UIImage imageNamed:@"noorderImage"] withString:@"此状态还没有订单信息~"];
...
//        popView = [XbbPopView initWithItems:@[NSLocalizedString(@"orderStateZone", nil),NSLocalizedString(@"orderStateOne", nil),NSLocalizedString(@"orderStateTwo", nil),NSLocalizedString(@"orderStateThree", nil),NSLocalizedString(@"orderStateFour", nil),NSLocalizedString(@"orderStateFive", nil),NSLocalizedString(@"orderStateSix", nil),NSLocalizedString(@"orderStateSeven", nil),NSLocalizedString(@"orderStateAll", nil)]];
